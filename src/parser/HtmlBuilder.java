...
public class HtmlBuilder {
    ...
    private void enterDescription() {
        stack.add(actualBuilder = builderDes);
        genericEnter();
    }

    private void enterParameters() {
        stack.add(actualBuilder = builderPar);
        genericEnter();
    }

    private void enterReturn() {
        stack.add(actualBuilder = builderRet);
        genericEnter();
    }

    private void enterThrows() {
        stack.add(actualBuilder = builderThr);
        genericEnter();
    }

    private void enterException() {
        stack.add(actualBuilder = builderExc);
        genericEnter();
    }

    private void enterSeeAlso() {
        stack.add(actualBuilder = builderSee);
        genericEnter();
    }
    ...
    public long getHash() {
        return Utils.getDocHtmlHash(html);
    }
        ...
        if (null != tag.getTagName()) {
            switch (tag.getTagName()) {
                ...
                case TagElement.TAG_DEPRECATED:
                    enterOther("Deprecated:");
                    pop = true;
                    break;
                case TagElement.TAG_SERIAL:
                    enterOther("Serial:");
                    pop = true;
                    break;
