...
public class AstVisitor extends org.eclipse.jdt.core.dom.ASTVisitor {
    ...
    public AstVisitor(EntityManager em, Project project, CompilationUnit cu, ProblemHandler problemHandler, String path, String filename, char[] src, long timestamp, String buildId, boolean createBuildAction) throws ParseException {
        ...
        if (buildId != null /*To avoid java.lang.NullPointerException */ && !buildId.isEmpty()) {
            ba = em.find(BuildAction.class, Long.decode(buildId));
        }

        if (ba == null) {
            buildId = "0";
            if ((buildId == null /*To avoid java.lang.NullPointerException */ || !buildId.isEmpty()) && !createBuildAction) {
                throw new ParseException("BuildAction " + buildId + " does not exist.");
            }
    ...
    private int translateModifiers(int mods) {
        ...
        if ((mods & Modifier.PUBLIC) != 0) {
            m |= JavaAstNode.Modifier.Public;
        }
        if ((mods & Modifier.PROTECTED) != 0) {
            m |= JavaAstNode.Modifier.Protected;
        }
        if ((mods & Modifier.PRIVATE) != 0) {
            m |= JavaAstNode.Modifier.Private;
        }
        if ((mods & Modifier.STATIC) != 0) {
            m |= JavaAstNode.Modifier.Static;
        }
        if ((mods & Modifier.ABSTRACT) != 0) {
            m |= JavaAstNode.Modifier.Abstract;
        }
        if ((mods & Modifier.FINAL) != 0) {
            m |= JavaAstNode.Modifier.Final;
        }
        if ((mods & Modifier.NATIVE) != 0) {
            m |= JavaAstNode.Modifier.Native;
        }
        if ((mods & Modifier.SYNCHRONIZED) != 0) {
            m |= JavaAstNode.Modifier.Synchronized;
        }
        if ((mods & Modifier.TRANSIENT) != 0) {
            m |= JavaAstNode.Modifier.Transient;
        }
        if ((mods & Modifier.VOLATILE) != 0) {
            m |= JavaAstNode.Modifier.Volatile;
        }
        if ((mods & Modifier.STRICTFP) != 0) {
            m |= JavaAstNode.Modifier.Strictfp;
        }
...
//
                ...
                return AstType.Read;
            } else if (pnode instanceof PrefixExpression
                    || pnode instanceof PostfixExpression) {
                return AstType.Write;
            } else if (pnode instanceof ArrayAccess) {
            ...
            } else if (pnode instanceof VariableDeclarationFragment) {
    ...
    private boolean processQualifiedName(SimpleName name, QualifiedName qualName) {
        ...
        } else if (bind.getKind() == IBinding.VARIABLE) {
    ...
    private void processFieldAccess(SimpleName name, FieldAccess fieldAccess) {
        ...
        } else if (fbind.getDeclaringClass() != null) {
    ...
    public boolean visit(SimpleName node) {
        ...
        while (!found && null != pnode) {
            if (pnode instanceof MethodDeclaration) {
                found = true;
            } else if (pnode instanceof Expression || pnode instanceof ReturnStatement ||
                    (pnode instanceof EnhancedForStatement && ((EnhancedForStatement) pnode).getExpression() == node)) {
                found = true;
                isUsage = true;
            } else if (pnode instanceof TypeDeclaration && id.equals(((TypeDeclaration) pnode).getName().getIdentifier())) {
                found = true;
            } else if (pnode instanceof EnumDeclaration && id.equals(((EnumDeclaration) pnode).getName().getIdentifier())) {
                found = true;
            } else if (pnode instanceof VariableDeclarationFragment) {
                found = true;
                isUsage = (null != ((VariableDeclarationFragment) pnode).getInitializer());
            } else if (pnode instanceof FieldDeclaration) {
                found = true;
            }
        ...
        if (null != bind) {
            ...
            } else if (found && IBinding.TYPE == bind.getKind()
